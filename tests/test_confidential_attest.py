...
def world():
    ...
    rd = b"\x11" * 32 + hashlib.sha256(spki).digest()
    ...
    return {"key": key, "cert": cert, "pem": pem.decode(), "rd": rd, "inst": inst, "ref": ref, "inner": inner}


def test_known_positive_verifies_every_check(world):
    r = A.verify_instance(world["inst"], NONCE, world["ref"])
...
def test_wrong_nonce_is_refused(world):
    r = A.verify_instance(world["inst"], "ff" * 32, world["ref"])
    assert not r.verified and r.failing == ["nonce_in_body"]
...
def test_tampered_body_and_wrong_key_signature_are_refused(world):
    ...
    assert not A.verify_instance(inst, NONCE, world["ref"]).checks["sig_ok"].ok
    other = rsa.generate_private_key(public_exponent=65537, key_size=2048)
    sig2 = base64.b64encode(other.sign(world["inner"], padding.PKCS1v15(), hashes.SHA256())).decode()
    assert not A.verify_instance(dict(world["inst"], signature=sig2), NONCE, world["ref"]).checks["sig_ok"].ok
...
def test_quote_not_bound_to_the_certificate_is_refused(world):
    ...
    r = A.verify_instance(dict(world["inst"], quote=q), NONCE, world["ref"])
...
def test_short_buffer_is_refused_not_crashed(world):
    r = A.verify_instance(dict(world["inst"], quote=base64.b64encode(b"\x00" * 10).decode()), NONCE, world["ref"])
...
def test_measurements_must_all_sit_in_one_registry_config(world):
    ...
    assert not A.verify_instance(dict(world["inst"], quote=q), NONCE, world["ref"]).checks["measurement_ok"].ok
...
def test_fleet_report_arithmetic_and_empty_fleet(world):
    ...
    fr = A.verify_fleet("chute-1", doc, world["ref"], NONCE)
...
def test_labels_cover_every_required_check_and_limitations_name_the_key_gap():
    ...
    assert any(k == "attributed-key" for k, _ in A.LIMITATIONS)
